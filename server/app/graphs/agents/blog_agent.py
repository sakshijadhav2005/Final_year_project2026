...
from app.graphs.tools import search_web
from app.core.config import get_settings

logger = structlog.get_logger("blog_agent")
...
class BlogAgent(BaseCreatorAgent):
    ...
    async def generate(
        ...
        try:
            ...
            # 2. Bind our tools (The Web Researcher)
            tools = [search_web]
            ...
            user_prompt = (
                ...
                "CRITICAL: If the transcript makes any claims about statistics, AI, or industry trends, "
                "you MUST use your `search_web` tool to find a real, recent citation to back it up!"
            )
            
            # 5. Execute the Agent loop! It will automatically call tools if needed.
            logger.info("blog_agent_started_with_tools")
            ...
            return {"type": "blog", "title": "EventAI Blog", "body": final_text, "structured": {}}
            
        except Exception as exc:
            logger.error("blog_agent_tool_error", error=str(exc))
